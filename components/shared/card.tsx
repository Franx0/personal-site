    ...
    <AnimateSharedLayout>
      {
        cardOpened ? (
          <motion.div className={`md:p-12 content-center expanded-card ${className}`} layoutId="expandable-card" style={expandedStyle}>
            <button className="absolute top-0 right-0 text-right p-3 text-current mr-1 cursor-pointer" title="Close" type="button" onClick={() => {logEvent({category: image.alt, action: "Close Card", label: "text"}); handleCardOpened("#")}}>X</button>
          ...
          <motion.a title={image.alt} onClick={() => {logEvent({category: image.alt, action: "Open Card", label: "text"}); handleCardOpened()}} className={`shadow-lg cursor-pointer normal-card ${className}`} layoutId="expandable-card" style={style}>
