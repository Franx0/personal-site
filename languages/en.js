...
export default {
  ...
  cv: {
    ...
    jobs: {
      ...
      "Glownet": {
        description: `I did not enjoy my consultancy experience as much as I expected so I moved on to a new and amazing project. In Glownet we developed a RFID system to run multiple events with no need of internet connection. Sònar, Coachella, MadCool and many others trusted on Glownet to run their festivals. The system were built by two different teams. The Android team developed the Mobile App to control onsite everything that happened inside the events and the backend team developed the neccesary APIs to do so and the adminsitration tool to control everything from there and to check all the analytics created during the events. Again Ruby on Rails was the chosen technology to do that, with a powerfull database system based on PostgreSQL and the servers running in Amazon AWS.`,
